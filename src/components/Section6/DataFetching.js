...
/*
  Tutorial - 1 - Data fetching using setState() and useEffect()
    ...
    1. If axios is not installed, then install axios using : npm i axios (check package.json for version).
    ...
    4. Inside useEffect() function, make a get request using axios.
        .then() block will provide us the responcse, which we can store in the state variable.
...
/*
  Tutorial -2 - Fetching Data from a specific endpoint using useEffect() hook
    ...
    2. Since we now are fetching only one object, we will initialise setState() with an empty object.
    ...
    4. In useEffect(), we will use get method from axios to fetch the data.
      ...
      .catch() vlock will provide us info about errors if any.
