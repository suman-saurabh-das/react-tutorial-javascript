...
function IntervalHookCounter() {
    ...
    useEffect(() => {
        ...
        doSomething()
        // So it is recommended that when we use some function inside useEffect, we should call it inside useEffect, this way we are much likely to see that there is a dependency on someProp
        function doSomething() {
            console.log(someProp);
        }
        ...
        // Here by specifying an empty array, we have basically told react to ignore watching for changes in the count variable. So on initial render, count is 0 and setCount will set it to 0 + 1 which is 1 and that will be rendered in browser, but since the dependency array is empty, react will not watch for changes in the count value. Count value is currently 1 and it will render the same value in different render cycles.
    // }, [])

    // If we add count as a dependency, we will get the expected result.
    // }, [count])

    // Adding the props to dependency array
    }, [someProp])
